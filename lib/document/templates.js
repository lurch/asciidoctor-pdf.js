...
const fontAwesomeStyleContent = (node) => {
  if (node.getDocument().isAttribute('icontype', 'svg')) {
    return faDom.css()
  }
  return ''
}
...
${syntaxHighlighterFooter(node, syntaxHighlighter, { cdn_base_url: cdnBaseUrl, linkcss: linkcss, self_closing_tag_slash: '/' })}
  ...
  inline_image: (node, baseConverter) => {
    if (node.getType() === 'icon' && node.getDocument().isAttribute('icontype', 'svg')) {
      const transform = {}
  ...
  colist: (node) => {
    ...
    if (node.getDocument().hasAttribute('icontype')) {
      result.push('<table>')
      let num = 0
      const svgIcons = node.getDocument().isAttribute('icontype', 'svg')
      let numLabel
